...
PATCH = config.PREDICTION_PATCH_SIZE          # 256 (recommended for your current model)
...
def build_valid_mask(image_np: np.ndarray, nodata_value, nodata_eps=0):
    ...
    if nodata_value is not None:
        ...
        # fallback: treat all-zero as nodata (common in EO)
...
def s1_downsample_10m_to_20m(s1_10m: torch.Tensor) -> torch.Tensor:
    ...
    """
    # Avg pooling is a good default for SAR composites
...
def normalize_s1_and_make_3ch(s1_np: np.ndarray, nodata_value=None) -> torch.Tensor:
    """
    Input: raw S1 patch at 10m grid (aligned), shape (2, H, W) where H=W=PATCH (256).
    ...
    # clamp negatives (SAR should be >=0 in your case)
    s1 = torch.clamp(s1, min=0.0)

    # log space like in training
    ...
    # make the 3rd channel exactly like your dataloader:
    # VH=band0, VV=band1
    vh = s1[0:1]              # (1,128,128)
    vv = s1[1:2]              # (1,128,128)
...
def predict_tile(model, s2_np, s1_np, s2_nodata, s1_nodata, nodata_eps=0, out_nodata_value=None):
    ...
    s1_np: (2,  H, W) raw 10m (ALREADY aligned to S2 grid)
    ...
    s2 = normalize_s2(s2_np)  # (C2,H,W) torch float

    # --- keep S1 raw as torch; we will normalize per patch (so pooling lines up) ---
    ...
    for i in range(0, H - PATCH + 1, STRIDE):
        for j in range(0, W - PATCH + 1, STRIDE):
            ...
            # --- S1 preprocessing exactly like training logic ---
...
def main():
    ...
    s1_path = config.PREDICTION_INPUT_S1_ALIGNED  # <-- add this to config
    ...
    with rasterio.open(s2_path) as s2_src, rasterio.open(s1_path) as s1_src:
        ...
        out_nodata_value = float(s2_nodata) if s2_nodata is not None else -9999.0
        out_profile.update(count=1, dtype=rasterio.float32, nodata=out_nodata_value)
