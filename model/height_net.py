...
class Sentinel2ResUNet(nn.Module):
    ...
    def forward(self, s2, s1):
        """
        s2: (B, 10, 256, 256)
        s1: (B, s1_in_channels, 256, 256)
        """

        # --- Encoder level 1 ---
        e1a = self.enc1a(s2)
        e1b = self.enc1b(s2)
        e1_s2 = e1a + e1b                        # (B,64,256,256)

        # --- S1 full-res features ---
        e1_s1 = self.s1_stem(s1)                 # (B,64,256,256)

        # --- Full-resolution fusion ---
        e1 = self.fuse1(torch.cat([e1_s2, e1_s1], dim=1))  # (B,64,256,256)
